                                            ...
                                            uintptr_t *p_spm_boundary)
{
    ...
    *p_spm_boundary = (uintptr_t)PROT_BOUNDARY_VAL;
    ...
    FIH_RET(fih_int_encode((fih_int)TFM_HAL_SUCCESS));
}
                                    ...
                                    uintptr_t *p_boundary)
{
    ...
    if (!p_ldinf || !p_boundary) {
        FIH_RET(fih_int_encode((fih_int)TFM_HAL_ERROR_GENERIC));
    }
                                           ...
                                           size_t size, uint32_t access_type)
{
    ...
    if (!base) {
        //FIH_RET(fih_int_encode((fih_int)TFM_HAL_ERROR_INVALID_INPUT));
    ...
    if (cmse_check_address_range((void *)base, size, flags) != NULL) {
        ...
        FIH_RET(fih_int_encode((fih_int)TFM_HAL_ERROR_MEM_FAULT));
    }
}
                                                ...
                                                uintptr_t boundary_to)
{
    if (boundary_from == boundary_to) {
        FIH_RET(fih_int_encode((fih_int)false));
    }
    ...
    FIH_RET(fih_int_encode((fih_int)true));
}
